...
module.exports = {
  ...
  try {
    ...
    const now = new Date();
    const currentWIBDate = new Date(now.getTime() + 7 * 60 * 60 * 1000);
    const currentDate = currentWIBDate.toISOString().split("T")[0];
    const currentTime = currentWIBDate.toISOString().split("T")[1].split(".")[0];
    ...
    const result = await prisma.$queryRaw`
      ...
      flight_search AS (
        ...
        WHERE
          ...
          ${departure_date === currentDate ? Prisma.sql`AND f.departure_time >= ${currentTime}` : Prisma.sql``}
      )
      SELECT
        *
      FROM
